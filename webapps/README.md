...
- `preview_app.js` — renders the card preview chart; calls `window.WSBPreviewShared.initThemeSync()` and `createAutoRefresher()`
...
- `shared/preview_shared.js`
  ...
  - Exposes `window.WSBPreviewShared.createAutoRefresher(options)` — handles polling interval, wake events (`visibilitychange`, `focus`, `pageshow`, `online`), and `refreshInFlight` guard
