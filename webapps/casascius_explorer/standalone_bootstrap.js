(() => {
  const STANDALONE_FILENAME = "casascius_explorer.png";
  const IMAGE_LIST_URL = "assets/image_list.json";
  const DASHBOARD_URL = "webapps/casascius_explorer/dashboard.html";
  ...
  function getStandalonePath() {
    ...
    const path = IS_LOCAL_HOST
      ? `${base}/casascius_explorer.html`
      : `${base}/casascius_explorer`;
  ...
  function getMainRouteUrl(filename) {
    ...
    if (slug === "casascius_explorer") return getStandalonePath();
  ...
  function setCurrentImage(image, index) {
    ...
    document.title = `${currentImage.title || "Casascius Explorer"} | Wicked Smart Bitcoin`;
    if (modalImg) {
